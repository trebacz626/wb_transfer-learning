...
if __name__ == "__main__":
    ...
    for epoch in range(
        offset + opt.epoch_count, offset + opt.n_epochs + opt.n_epochs_decay + 1
        ...
        for i, data in enumerate(train_dataset):  # inner loop within one epoch
            ...
            loss = criterion_dice(pred_labels, labels) + criterion_ce(
                pred_labels, labels
            )
            loss = loss.mean()
            loss.backward()
                ...
                visualizer.log_images(
                    {
                        ...
                        "labels": torch.unsqueeze(
                            torch.argmax(labels, dim=1) / 3.5 - 1, dim=1
                        ),
                        "pred_labels": torch.unsqueeze(
                            torch.argmax(pred_labels, dim=1) / 3.5 - 1, dim=1
                        ),
                    }
                )
        ...
        if epoch % opt.save_epoch_freq == 0:
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(valid_dataset):
                    scan, labels = data["T_scan"].to(device), data["T_labels"].to(
                        device
                    )
                    pred_labels = model(scan)
                    scores = {
                        ...
                        "dice_hard": 1
                        - criterion_dice(pred_labels, labels, one_hot_encode=True),
                    }
                    for key in scores:
            ...
            if valid_scores["dice_hard"] > best_validation_score:
                ...
                torch.save(model.state_dict(), f"unet-checkpoint-{epoch}"),

        print(
            "End of epoch %d / %d \t Time Taken: %d sec"
            % (epoch, opt.n_epochs + opt.n_epochs_decay, time.time() - epoch_start_time)
        )
    torch.save(model.state_dict(), f"unet-checkpoint-final"),
